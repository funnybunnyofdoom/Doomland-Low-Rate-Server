...
public class TreeEntity : Tree, IInteractableObject, IDamageable, IMinimapObject
{
    ...
    public override float GrowthPercent
    {
        ...
        set
        {
            ...
            while (this.Species != null && !this.Fallen && this.CurrentGrowthThreshold < GrowthThresholds.Length && value >= GrowthThresholds[this.CurrentGrowthThreshold])
            {
                var block = World.GetBlock(this.Position.Round + Vector3i.Up * (this.CurrentGrowthThreshold + 1));
                if (!block.Is<Empty>() && block.GetType() != this.Species.BlockType)
                {
                    this.growthPercent = GrowthThresholds[this.CurrentGrowthThreshold] - 0.01f;
    ...
    public static bool TreeRootsBlockDigging(InteractionContext context)
    {
        return EcoSim.PlantSim.AnyPlantsWithinRange(context.BlockPosition.Value + Vector3i.Up, 1.5f, x =>
            x is TreeEntity
            && (x as TreeEntity).stumpHealth > 0f 
            && Math.Round(x.Position.y) > context.BlockPosition.Value.y 
            && x.GrowthPercent > saplingGrowthPercent
            && MathUtil.Lerp(0.5f, 1.5f, x.GrowthPercent * 2f) >= Vector3.Distance(x.Position, context.BlockPosition.Value) // covers 3x3 at 50%
        );
    }
    ...
    private bool TryDamageUnfelledTree(INetObject damager, float amount, InteractionContext context)
    {
        if (this.health > 0)
        {
            ...
            {
                IAtomicAction statAction = PlayerActions.Harvest.CreateAtomicAction((Player)damager, this);
    ...
    private bool TryDamageStump(INetObject damager, float amount, InteractionContext context)
    {
        if (this.Fallen && this.stumpHealth > 0)
        {
            ...
            if (damager is Player)
            {
                var action = PlayerActions.RemoveStump.CreateAtomicAction((Player)damager, this);
                actions.Add(action);
