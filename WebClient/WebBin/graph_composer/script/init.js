(function()
{
		...
		if(isFrontPage) {
			...
			$.get(url, function(data, status){
				...
				if(graphName == "skills") {
					...
					$('#subheader').html("Player Skill Progress");
					...
					$('#subheader').html("Pollution Progress");
					graph.setTimeRange(categories[1].time_start, categories[1].time_end);
					for(var i = 0; i < categories[1].Keys.length; i++) {
						graph.addDataset(categories[1].Keys[i].join("/"));
					}
	...
	function onClickGraphSave() {
		...
		for( var key in graphList ) {
			if ( key != graph.id && graphList[key].toUpperCase() == name.toUpperCase() ) {
				Popups.info( "A graph with that title already exists! Choose a unique title for your graph." );
				return false;
			}
		}
		...
		if( !graph.save(updateGraphList) ) {
			// can't save
			Popups.info("Please add a dataset to your graph before saving.");
			return false;
		}
		return true;
	}
	...
	function onClickGraphCopy() {
		promptToSaveChanges(function(){
			...
			Popups.info("Graph copied. Type a unique name and save it.", "good");
		});
	}
	...
	function promptToSaveChanges( onProceed ) {
		if(graph.modified) {
			var fields = {
				...
				body: "Do you want to save your changes before proceeding?",
				btn0: "Don't Save",
				btn1: "Save"
			};
	...
	function createObjByDataset( data ) {
		...
		for(var n in data ) {
			if( data[n].indexOf("/") != -1 ) {
				...
				var arr = data[n].split("/");
				if( subcat != arr[0] ) {
					subcat = arr[0];
					dataObj[subcat] = [];
					j = 0;
				}
				dataObj[subcat][j++] = data[n].slice( data[n].indexOf("/") + 1 );
				t = true;
			} else {
				dataObj[ i++ ] = data[n];
			}
		...
		if( t ) {
			for(var n in dataObj) {
				dataObj[n] = createObjByDataset( dataObj[n] );
			}
		}
	...
	function createDatasetList( data, category ) {
		...
		for( var n in data ) {
			if(typeof data[n] == "object") {
				...
				var test = $("<div><h2> "+ n +"<span class='count nested'></span> </h2></div>").attr(attrCategory);
		...
		for( var n in data ) {
			if(typeof data[n] != "object") {
				list.append("<li data-icon='plus'><a class='miniListItem ui-icon-plus ui-btn ui-btn-icon-right' title='"+data[n]+"'data-parent='"+category+"' href='#'>" + data[n] + "</a></li>");
			}
		}
		return list;
	}
	...
	function updateDatasetList() {
		...
		var data = datasets;
		$.ajax({
			url: urlGameServer + "/datasets/list",
			...
			success: function( data, status, xhr ) {
				...
				for( var category in data ) {
					...
					var ulData = createObjByDataset(data[category]);
					var list = createDatasetList(ulData, category);
					cat.append(list);
	...
	function getDatasetFromMenu( menu ) {
		var group = menu.parents('div.ui-collapsible');
		var cat = group.find('h2 a').contents().filter(function(){
			return this.nodeType == 3;
		})[0].nodeValue;
		cat = menu.data('parent');
		return cat.trim() + "/" + menu.text().trim();
	}
