(function(){
	...
	*/
	this.showError = function(message, type){
		...
		if(typeof type == "undefined"){
			$('.error').empty().css('background-color', '#D4784B').html("Error: " + message).fadeIn();
			$('html,body').animate({
				scrollTop: '0'
			}, "slow");
	...
	*/
	...
	this.removeComment = function(comment, hasgraph){
		...
		var url = APPIQUERYURL + "/api/v1/elections/removecomment?username=" + election.username + "&authtoken=" + election.authtoken;
		$.ajax({
			url: url,
			type: 'POST',
			data: data,
	...
	*/
	this.addComment = function(comment, isEdited){
		...
		if(comment){
			...
			var url = APPIQUERYURL + "/api/v1/elections/addComment?authtoken="+election.authtoken + "&username="+election.username;
			...
			$.ajax({
				...
				error: function(error){
					if(typeof error.responseJSON != 'undefined'){
						this.showError(error.responseJSON.ExceptionMessage);
					}
				}
			})
	...
	*/
	this.vote = function(){
		...
		var url = APPIQUERYURL + "/api/v1/elections/vote?username=" + election.username + "&authtoken=" + election.authtoken;
		...
		var settings = {
			...
			$.ajax(settings).done(function (response) {
				...
				var error = err.responseJSON.ExceptionMessage;
		...
		// updating election info
				...
				if( data.Candidates.length != 0 ){
					...
					if(isPrevious != 0 || typeof guid != typeof undefined){
						info += "<li>Ended  " + prevEnded +" ago</li>";
						info += "<li>" ;
						info += (data.Results.Winner != "") ? "The winner was " + data.Results.Winner + ".</li>" : "There was no winner.</li>";
					}
				...
				$('.current-rankings table tr:first-of-type td').html("All Votes");
				...
				$('.section-title-results p').empty().append(res);

				info += "<li>" + data.TotalVotes + (data.TotalVotes==1 ? " vote": " votes")+" total. </li>";
			...
			/*	if(isPrevious == 0 && typeof guid == typeof undefined){
				...
				if(isPrevious == 0 && typeof guid == typeof undefined) {
					info += "<li><b>Projected result:</b> "+ data.Results.Result + leaderReplace + "</li>";
				}
				...
				if(isPrevious == 0 && typeof guid == typeof undefined) {
					var info2 = "<li>Rank the candidates from best to worst.</li>";
				}
	...
	*/
	this.showCandidates = function(guid){
		...
		function showCandidates(data){
			...
			var row = $("<tr class='table-top'> <td class=' table-top-left'> Candidates </td> </tr>");

			for(var i = 0; i < rounds; i++ )
			{
				row.append("<td>Round  "+(i+1)+"</td>");
			}
			...
			var total = $("<tr class='table-bottom'> <td> Total</td> </tr>");
			...
			function tableShow(data){
				...
				data.forEach(function(item, index){
					if(item.Voter == election.username){
						$('.vote-button').attr('disabled', 'disabled').html('Update Vote');
					}
				})
	...
	*/
	this.removeSelf = function(){
		...
		var url = APPIQUERYURL + "/api/v1/elections/removecandidate?authtoken=" + election.authtoken + "&username=" + election.username + "&speech=" + speech;
		...
		for(var i = 0 ; i <= cycleLength + 1; i++)
		{
			...
			for(var j = 0; j <= newCycleLength; j++)
			{
				if(i == 0 && j == 0){
					...
					row.append("<td class='table-key table-top-left'> &nbsp; </td> <td class='table-key' colspan='"+candidates.length+"'> Candidates Ranked </td>")
					...
					if(i == 1){
						...
						row.append("<td class='table-top table-top-left table-key'>Voters</td>");
