...
var Law = function(id){
	...
	*/
	this.getAllLaw = function(callback,update){
		update = update || false;
		var url = APPIQUERYURL + '/api/v1/laws?username=' + law.username + '&authtoken=' + law.authtoken,
			_this = this;
	...
	*/
	this.getAllComments = function(callback,update){
		var url = APPIQUERYURL + '/api/v1/laws/getcomments?lawguid=' + law.currentLawID + '&username=' + law.username + '&authtoken=' + law.authtoken,
			_this = this;
	...
	*/
	this.removeComment = function(comment){
		...
		var url = `${APPIQUERYURL}/api/v1/laws/removecomment?authtoken=${law.authtoken}&username=${law.username}&lawguid=${law.currentLawID}`;
	...
	*/
	this.GetComments = function(type){
			type = type || false;
	        // $.get(APPIQUERYURL + '/api/v1/laws/getcomments?lawguid=' + law.currentLawID + '&username=' + law.username + '&authtoken=' + law.authtoken, function(data) {
            ...
            for (var i = 0; i < data.length; i++) {
				...
				if (/^[\],:{}\s]*$/.test(data[i].Text.replace(/\\["\\\/bfnrtu]/g, '@').
				replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g, ']').
				replace(/(?:^|:|,)(?:\s*\[)+/g, ''))) {

					comment = JSON.parse(data[i].Text);
					// Editing feature to do
					//isEdited = JSON.parse(data[i].IsEdited);
				...
				section.append("<div class='law-discussion-user'>" + data[i].Username + "<span class='law-discussion-time'>" + commentTime +((data[i].IsEdited) ? "&nbsp;<i>(edited)</i>" : "")+"</span></div><div class='law-discussion-comment'>" + comment + "</div>");
				text.append(section);
	...
	*/
	this.addComment = function(comment, isEdited){
		...
		if(this.username == null || this.authtoken == null){
			this.showError("To add comment you need to access from the game server");
			return false;
		}

		if(this.inEffect){
			this.showError("You can't add comment for Proposed Law.")
			return false;
		}
		...
		if ( ($('#comment').val() != null && $('#comment').val() != "") || comment != null) {
			...
			var url = APPIQUERYURL + '/api/v1/laws/addcomment?authtoken=' + law.authtoken + '&username=' + law.username + '&lawguid=' + law.currentLawID;
			$.post(url, data, function(r) {
				...
				if(typeof error.responseJSON != 'undefined'){
					...
					$('#status').empty().html("Error: " + error.responseJSON.ExceptionMessage).fadeIn();
				}
			});
		}
	}
  ...
  return text
	...
	*/
	this.sendVote = function(vote){
	    ...
	    var url = APPIQUERYURL + "/api/v1/laws/" + currentLawID + "/vote/" + ((vote == 0) ? 'true' : 'false') + "?username=" + law.username + '&authtoken=' + law.authtoken;
	    ...
	    $.ajax({
	        ...
	        error: function(a, b, c) {
	            ...
	            $('#status').empty().html(error.ExceptionMessage).fadeIn();
	        }
	    })
	}
