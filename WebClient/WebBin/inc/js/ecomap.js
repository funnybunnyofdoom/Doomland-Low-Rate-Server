(function (global, factory) {
  ...
  (factory((global.ECO = global.ECO || {})));
	...
	function GIFDECODER(name, OnFinish) {
	...
	var GIFMANAGER = (function() {
	    ...
	    my.giftextures = function(name, onFinish) {
	        if(!my.gifStorage[name]) {
	            ...
	            my.gifStorage[name] = [];
	            my.gifTimes[name] = [];
	            var tempLoader = new GIFDECODER(name, onFinish?onFinish:my.OnGifLoaded);
	...
	var MINIMAP = (function () {
	    ...
	    my.init = function () {
	        var texture = new THREE.TextureLoader().load('Layers/HeightMap.gif', function (texture) {
	    ...
	    my.addView = function (view) {
	        ...
	        if (undefined === view.settings.frameNum) view.settings.frameNum = 0;
	        ...
	        if (undefined === view.settings.currentTime) view.settings.currentTime = 0.0;
	        if (undefined === view.settings.flat) view.settings.flat = false;
	        if (undefined === view.settings.pause) view.settings.pause = false;
	    ...
	    function render() {
	        ...
	        scenes.forEach(function (scene) {
	            ...
	            if (!s.pause && s.playSpeed != 0.0 && GIFMANAGER.gifTimes[s.layerSelected] != undefined && GIFMANAGER.gifTimes[s.layerSelected].length > 0) {
	                var myTimes = GIFMANAGER.gifTimes[s.layerSelected];
	                var maxFrame = myTimes.length;
	                if (myTimes === undefined) maxFrame = 0;

	                s.currentTime += ((view.loading ? 0.2 : s.playSpeed) * 86400) * deltaTime;
	                if (view.loading && maxFrame > 2 && s.currentTime > myTimes[maxFrame - 2])
	                    s.currentTime = myTimes[maxFrame - 2];
	                if (s.currentTime > s.timeEnd) {
	                    s.currentTime = s.timeStart;
	                    s.frameNum = 0;
	                } else if (s.currentTime < s.timeStart) {
	                    s.currentTime = s.timeEnd;
	                    s.frameNum = maxFrame - 1;
	                }

	                while (s.playSpeed > 0 && s.frameNum < maxFrame - 1 && s.currentTime > myTimes[s.frameNum + 1])
	                    s.frameNum++;
	                while ((s.playSpeed < 0) && (s.frameNum > 0) && s.currentTime < myTimes[s.frameNum])
	                    s.frameNum--;
	            ...
	            if (mouseViewX > 0 && mouseViewY > 0 && mouseViewX < rect.width && mouseViewY < rect.height || view.firstRender === undefined) {
	            ...
	            if (GIFMANAGER.gifStorage[s.layerSelected])
	            ...
	            cam.near = cam.position.y - 2550.0;
	            cam.far = cam.position.y + worldWidth * 100;
