...
function lawspage(lawID,curentLaw,Law,action='load'){
	...
	function law_load(lawsData){
		...
		var data = lawsData;
				...
				$('#laws-page-wrapper').css('min-height', '712px');
					...
					var ratifedDay;
					var ratifedHour;
					var ratifedEnding = " ago";
					if(days == 1 ){
						ratifedDay = "1 day ";
					}
					else if (days > 1 ){
						ratifedDay = days + " days ";
					}else{
					...
					if(hours == 1 ){
						...
						ratifedHour = hours + " hrs";
					}
					else if ( (hours < 1 ) && (days < 1 ) ){
						ratifedHour = "in the last hour";
						ratifedEnding = "";
					}

					$('#timeOfVote').text(ratifedDay+ratifedHour+ratifedEnding);

					var description = checkedData['Description'];

					/* checking if "description" is a valid JSON string  */

					if (/^[\],:{}\s]*$/.test(description.replace(/\\["\\\/bfnrtu]/g, '@').
					...
					$('#law-reasoning-description-graph p').html(description);
						...
						if( checkedData['LawText'].indexOf("districtEditor") != -1 && $(".district-controls-wrapper").length == 0 ) {
							...
							variables.getLawDistrict( currentLawID, fieldId, function(obj){
								...
								for(var i in obj.data.DistrictMetadata) {
									...
									if( d.ID == 0 ) continue;
									$(".district-wrap").before('<div class="district-item proposed-districts" data-id="'+d.ID+'" data-color="'+JSON.stringify(d)+'" style="background:'+rgb+'">'+d.Name+'</div>');
								}
								for(var i in obj.currentDistricts.DistrictMetadata) {
									var d = obj.currentDistricts.DistrictMetadata[i];
									var rgb = "rgb("+d.R+","+d.G+","+d.B+")";
									if( d.ID == 0 ) continue;
									$(".district-wrap").before('<div class="district-item current-districts" data-id="'+d.ID+'" data-color="'+JSON.stringify(d)+'" style="background:'+rgb+';display:none;">'+d.Name+'</div>');
								}
					...
					if(checkedData['ExistingLawGuid'] && !checkedData['InEffect']){
						...
						$('.law-conflict').append("If passed, this law will replace <a href='?username=" + username + "&authtoken=" + auth + "&lawid="+checkedData['ExistingLawGuid']+"'> "+ checkedData['ExistingLawTitle'] +" </a>");
					}


					if(checkedData.ProposingUser == user && !checkedData.InEffect){
						$('.section-law-info .cancel-law').remove();
						$('.section-law-info').append("<button class='cancel-law global-button global-button-small'>Delete Proposal</button>");
					}
					...
					$('#page-wrapper').on('click', '.cancel-law', function(){
						$.ajax({
							type: 'POST',
							url: APPIQUERYURL+"/api/v1/laws/cancellaw?username="+ user +"&authtoken="+ auth +"&guid=" + currentLawID,
							success: function(r){
	...
	function counter(t)
	{
		...
		function counterDisplay () {
			var displayhours = hours + " hr ";
			var displayminutes = minutes + " min ";
			...
			var counter = "Vote ends in " + displayhours + displayminutes + "<div class='countdown-time'>" + displayseconds + "</div> sec ";
